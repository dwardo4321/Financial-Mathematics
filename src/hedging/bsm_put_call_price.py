...
class black_scholes_pricer:

    def bsm_put_option_pricer(s_0: float, strike: float, sig: float, rate: float, T: int, alpha: float, n: int, risk_neutral_pricing: bool = False, plot: bool = False):
        ...
        utils = utilityDGE()
        c_tx, p_tx, delta_call, delta_put, bank_call, bank_put, portfolio_put, portfolio_call = utils.array_def(n)
    ...
    def hedging_error_dist(no_of_sims: int, parameters_call=None, parameters_put=None, parameters=None, plot: bool = False, c_p_same_paras: bool = False):
        ...
        if c_p_same_paras:
            ...
            for i in range(no_of_sims):
                data = bsm_put_option_pricer(s_0, strike, sig, rate, T, alpha, n, risk_neutral_pricing, plot = False)
            ...
            for i in range(no_of_sims):
                data_call = bsm_put_option_pricer(s_0_call, strike_call, sig_call, rate_call, T_call, alpha_call, n_call, risk_neutral_pricing_call, plot = False)
                data_put = bsm_put_option_pricer(s_0_put, strike_put, sig_put, rate_put, T_put, alpha_put, n_put, risk_neutral_pricing_put, plot = False)
